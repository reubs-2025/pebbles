...
void terminal_up() {
    ...
    for (int column = 0; column < VGA_WIDTH; column++) {
        terminal_buffer[(VGA_HEIGHT - 1) * VGA_WIDTH + column] = vga_entry(0x00, terminal_color);
    }
}
...
void terminal_putchar(char c) {
    if (c != 0x0A) {
        ...
        if (++terminal_column == VGA_WIDTH) {
            terminal_column = 0;
            if (++terminal_row == VGA_HEIGHT) terminal_up();
        }
    } else {
        terminal_column = 0;
        if (++terminal_row == VGA_HEIGHT) terminal_up();
...
void kernel_main(void) {
    ...
    terminal_writestring("PS/2 initialized successfully.");
    hlt_forever();
}
